...
# GET related pets records
@owner_bp.route('/<int:id>/pets', methods=['GET'])
def get_petss(id):
    items, errors = service.get_petss(id)

    # El servicio retorna (None, { 'error': ... }) si no encuentra la entidad padre
    if errors:
        # Podrías chequear si es "not found" u otro tipo de error
        if errors.get('error') == 'Owner not found':
...
@owner_bp.route('/<int:id>/pets', methods=['POST'])

def add_pets(id):
    data = request.get_json()
    item, errors = service.add_pets(id, data)
