...
class TestOwnerController(unittest.TestCase):
    ...
    This class contains test cases for the Flask controller handling Owner endpoints,
    including methods to test retrieval and creation of Owner entities.
    """
    ...
    def test_get_all_owners(self):
        """
        Test the GET endpoint for retrieving all Owner entities.

        Sends a GET request to the `/api/owners/` endpoint and verifies
        that the response status code is 200, indicating successful retrieval.
        """
        response = self.client.get('/api/owners/')  # Perform a GET request
    ...
    def test_create_owner(self):
        """
        Test the POST endpoint for creating a new Owner entity.

        Sends a POST request to the `/api/owners` endpoint with an empty JSON payload
        ...
        response = self.client.post('/api/owners/', json=payload)  # Perform a POST request
